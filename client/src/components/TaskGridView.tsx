...
export default function TaskGridView({ projectName, onTaskClick }: TaskGridViewProps) {
  ...
  const { data: tasks = [], isLoading } = useQuery<OperationalTask[]>({
    queryKey: [`/api/operational-tasks/${projectName}`],
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: [`/api/operational-tasks/${projectName}`] }),
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [`/api/operational-tasks/${projectName}`] });
      setNewTaskTitle("");
    },
  });
