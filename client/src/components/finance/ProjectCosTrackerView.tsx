/**
 * ProjectCosTrackerView — a project-scoped slice of the company COS tracker.
 *
 * Reads the SAME canonical endpoint as the COS page (`/api/cos-tracker`, FY-scoped)
 * and filters each month's per-project breakdowns to one project, exactly the way
 * the company GP page slices a project (finance-gp-company.tsx). It introduces NO
 * new calculation — every figure is a sum of the per-project values the endpoint
 * already returns, rendered through the shared <MoneyValue>. Used as the
 * "Cost of sales" tab on the project finance detail page.
 */
...
export function ProjectCosTrackerView({
  projectName,
}: {
  projectId: number;
  projectName: string;
  ...
  const { data, isLoading, isError, error, refetch } = useQuery<CosMonth[]>({
    queryKey: ["/api/cos-tracker", qs],
    queryFn: fetchQueryFn(`/api/cos-tracker?${qs}`),
    staleTime: 30_000,
  });

  const rows = useMemo<ProjectCosRow[]>(() => {
    return (data ?? []).map((m) => {
  ...
  // A project with no COS in any state across the window has nothing to show.
  const hasAny = rows.some((r) => r.realised !== 0 || r.committed !== 0 || r.planned !== 0);

  return (
    <div className="space-y-3" data-testid="project-cos-tracker">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="text-sm font-semibold text-foreground">Cost of sales — {projectName}</h3>
          <p className="text-xs text-muted-foreground">
            Monthly COS by recognition state ({fyScope.label}). Same canonical{" "}
            <span className="font-mono">/api/cos-tracker</span> figures as the COS page, sliced to this project.
          </p>
        </div>
      ...
      {isLoading ? (
        <FinanceLoading label="Loading project COS…" />
      ) : isError ? (
        <FinanceError hint={(error as Error)?.message} onRetry={() => void refetch()} />
      ) : !hasAny ? (
        <FinanceEmpty
          title="No cost of sales in this window"
          hint="This project has no realised, committed, or planned COS for the selected period."
        />
      ) : (
          ...
          <table className="w-full border-collapse text-sm">
            <thead className="bg-slate-50 sticky top-0">
              <tr className="border-b border-slate-200 text-[11px] uppercase tracking-wide text-slate-500">
                ...
                <th className="px-3 py-2 text-right font-semibold">Total COS</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.monthKey} className="border-b border-slate-100 last:border-0" data-testid={`project-cos-row-${r.monthKey}`}>
