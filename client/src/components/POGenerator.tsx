...
import {
  FileText, Plus, Trash2, Download, Package, Loader2,
  ChevronDown, ChevronUp, Copy, ArrowUp, ArrowDown, AlertCircle,
} from "lucide-react";
import {
                          ...
                          {po.status === "draft" && (
                            <>
                              ...
                              <Button
                                variant="outline"
                                size="sm"
                                disabled={!selectedApproverByPo[po.id] || submitForApprovalMutation.isPending}
                                onClick={() => submitForApprovalMutation.mutate({ poId: po.id, assignedApproverUserId: Number(selectedApproverByPo[po.id]) })}
                                data-testid={`btn-submit-po-${po.id}`}
                              >
                                {submitForApprovalMutation.isPending ? <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" /> : null}
                                Submit for Approval
                              </Button>
                              <Button
                                variant="destructive"
