...
import { ActivityTimeline, ProgramMonthlyOverlay, ProgramMonthlyTimeline } from "@/components/execution/activity-timeline";
import { TemplateControls } from "@/components/execution/activity-templates";
import {
  type MilestoneProgram, type MilestoneProgramRow, type ProjectMilestoneDetail, type MilestoneView,
  type LinkedTaskView, type FlowState, type TaskState, type ActivityTaskNode,
...
function ProgramOverview({ onOpen }: { onOpen: (id: number) => void }) {
  ...
  // Default: only projects with open money. Toggle in fully-settled projects too.
  const [showSettled, setShowSettled] = useState(false);
  const { data, isLoading, isError, refetch } = useQuery<MilestoneProgram>({
    queryKey: ["/api/milestone-tracker/program", showSettled ? "with-settled" : "open"],
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/milestone-tracker/program${showSettled ? "?includeSettled=true" : ""}`);
      return res.json();
    },
  });
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-4">
            ...
            <Button size="sm" variant={showSettled ? "default" : "outline"} onClick={() => setShowSettled((v) => !v)} data-testid="program-show-settled">Show settled</Button>
            {view === "list" && (
              <>
