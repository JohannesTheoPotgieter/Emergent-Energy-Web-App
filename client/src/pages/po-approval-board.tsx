...
export default function POApprovalBoardPage() {
  ...
  // UX-pass-1 rebuild (2026-05-27) — count badge per filter so urgency
  // is visible before the user clicks. Truth principle: don't hide
  // what needs attention behind a tab.
  ...
  return <PageLayout header={<PageHeader title="PO Approval Board" subtitle={`${allPos.length} purchase orders`} />}>
    <div className="flex items-center gap-1 flex-wrap">{FILTERS.map((f) => {
      ...
      // Tint "My Reviews" + "Requires Info" amber when there's work to
      // do — those are the only states where inaction blocks someone.
