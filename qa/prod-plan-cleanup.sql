...
--
-- Safety: reversible. Only parent_id, indent_level, deleted_at are changed,
--   plus a full backup table. Restore script is in SECTION 4.
--
...
--   (Restore in SECTION 4 reverts parent_id/indent_level/deleted_at to the
--   backup snapshot, so any concurrent edits to those 3 columns made between
...
-- ----------------------------------------------------------------------------
DROP TABLE IF EXISTS work_items_backup_20260603;
CREATE TABLE work_items_backup_20260603 AS SELECT * FROM work_items;
...
-- 2f. REVIEW THESE NUMBERS before committing.
SELECT
