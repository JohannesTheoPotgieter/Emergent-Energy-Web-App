...
async function ensureAtLeastOneProject(token: string): Promise<number> {
  const projects = await apiRequest("GET", "/api/projects", undefined, token);
  if (projects.status === 200 && Array.isArray(projects.data)) {
    for (const p of projects.data) {
      const id = p.project_info_id || p.id;
      if (id) return id;
    }
  }
  const createRes = await apiRequest("POST", "/api/projects", {
    projectName: `WF Bootstrap ${Date.now()}`,
    clientName: "Workflow QA",
    initialPhase: "P0_FIRST_ASSESSMENT",
  }, token);
  expect(createRes.status).toBe(200);
  const id = createRes.data?.project?.id;
...
describe("API: Critical workflow test pack", () => {
  ...
  it("Engineering task create -> update -> view -> delete, with invalid update guard", async () => {
    ...
    try {
      ...
      const projectId = await ensureAtLeastOneProject(token);
      ...
      // Prompt 0.9 / migration 20260413: canonical lowercase_snake.
  ...
  it("Generate engineering tasks for project handles create, re-read, duplicate-click protection, invalid project", async () => {
    ...
    try {
      ...
      if (!projectId) {
        ...
        // POST /api/projects now auto-generates the P0 stage's engineering
        // tasks during creation, so the project starts with eng tasks.
        createdFreshProject = createProject.data?.engStagesGenerated === true;
        log.step("create_fresh_project", { projectId, autoGenerated: createdFreshProject });
      }

      const generated = await apiRequest("POST", `/api/projects/${projectId}/generate-eng-tasks`, {}, token);
      if (createdFreshProject) {
        // Auto-generated on project create — second explicit generate is
        // expected to be blocked by the duplicate-click guard with 400.
  ...
  it("PD ticket flow create -> view -> spawn tasks with duplicate-submit protection", async () => {
    ...
    try {
      ...
      // PD tickets now require project linkage for lifecycle tracking.
      const projectIdForTicket = await ensureAtLeastOneProject(token);

      const dueDate = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString();
      const created = await apiRequest("POST", "/api/pd/tickets", {
        projectId: projectIdForTicket,
        projectSiteName: `WF PD Site ${Date.now()}`,
        requestType: "I&C",
        priority: "Medium",
        // PD tickets now require a dueDate for SLA tracking.
        dueDate,
      }, token);
      ...
      // Template task-spawn was retired; tickets no longer auto-spawn tasks.
      // Just assert the shape — `tasks` should be an array (may be empty).
      expect(Array.isArray(viewed.data?.tasks)).toBe(true);
      log.step("view_ticket_no_template_tasks", { taskCount: viewed.data.tasks?.length ?? 0 });

      // The retired spawn-tasks endpoint now returns 410 Gone.
      const retiredSpawn = await apiRequest("POST", `/api/pd/tickets/${ticketId}/spawn-tasks`, {}, token);
      expect(retiredSpawn.status).toBe(410);
      log.step("spawn_tasks_endpoint_retired", { status: retiredSpawn.status });
  ...
  it("Send for approval flow updates status and rejects invalid override", async () => {
    ...
    try {
      ...
      // Prompt 0.9 / migration 20260413 — canonical lowercase_snake
      // statuses replaced legacy ALL CAPS in work_items.
  ...
  it("My Work task flow create -> update -> view -> delete with invalid input guard and no duplicate on repeat update", async () => {
    ...
    try {
      ...
      // The newer mytool route returns 422 (VALIDATION_ERROR) for missing
      // required fields; legacy was 400. Accept either.
      ...
      expect(matches.length).toBe(1);
      // Migration 20260413 normalizes mytool task status to lowercase_snake.
      expect(matches[0].status).toBe("in_progress");
  ...
  // POST/DELETE /api/mytool/tasks moved out of `requireAdmin` — every
   // authenticated user owns their personal task list now, so the
   // "admin-only" framing this test was built on no longer applies.
   // Tracked as a follow-up to point this test at an endpoint that is
   // still strictly admin-only (admin/users, admin/control-center, ...).
  it.skip("Permission mismatch defense: non-admin is blocked from admin-only actions while admin succeeds", async () => {
