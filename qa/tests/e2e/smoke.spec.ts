...
const ROUTE_MATRIX: RouteExpectation[] = [
  {
    ...
    markers: {
      headingOrAnchor: [
        'text=/Welcome|Company Priorities|Exception Focus|My Work Preview/i',
        '[data-testid]',
      ],
      primaryWidgetOrAction: [
        "a[href='/company-priorities']",
        "a[href='/my-work/tasks']",
        "a[href='/my-work']",
      ],
    },
  },
  ...
  },
  {
    ...
    label: 'Teams Chat',
    markers: {
      headingOrAnchor: ['text=/Teams|Chat|Collaboration/i'],
      primaryWidgetOrAction: ['button', "a[href*='/my-work']", 'input'],
    },
  },
  {
    route: '/my-work',
...
function buildReport() {
  ...
  const missingRoutes = ROUTE_MATRIX.filter(
    (route) => !allResults.some((r) => r.route === route.route),
  );
  ...
  const rows = allResults
    .map(
      (r) =>
        `| ${r.route} | ${r.role} | ${r.status.toUpperCase()} | ${r.missingMarkers.length ? r.missingMarkers.join(', ') : '-'} | ${r.permissionBlocked ? 'yes' : 'no'} |`,
    )
    .join('\n');
  ...
  const suspiciousLines = suspicious.length
    ? suspicious
        .map(
          (r) =>
            `- ${r.route} (${r.role}): ${r.missingMarkers.join(', ') || 'missing stable anchors'}`,
        )
        .join('\n')
    : '- None';
...
test.describe('Expanded Route Smoke Coverage', () => {
  for (const [role, user] of Object.entries(TEST_USERS) as [RoleKey, TestUser][]) {
    test.describe(`Role: ${role}`, () => {
      test.skip(
        user.optional && (!user.username || !user.password),
        `Optional ${role} credentials not configured`,
      );
      ...
      for (const expectation of ROUTE_MATRIX) {
        test(`${role} route smoke: ${expectation.route}`, async ({ page }) => {
          ...
          await page
            ...
            .catch(() => notes.push('networkidle timeout'));
          await page.waitForTimeout(500);

          const finalUrl = new URL(page.url()).pathname;
          const permissionBlocked = await page
          ...
          const headingOrAnchorFound = await elementExists(
            page,
            expectation.markers.headingOrAnchor,
          );
          const primaryWidgetOrActionFound = await elementExists(
            page,
            expectation.markers.primaryWidgetOrAction,
          );

          const navSample = mainFrameNavs.slice(-12);
          const navUnique = new Set(
            navSample.map((u) => {
              try {
          ...
          if (!primaryWidgetOrActionFound)
            missingMarkers.push('expected primary widget/action missing');
          ...
          const suspiciousIncompleteView =
            !permissionBlocked && !headingOrAnchorFound && !primaryWidgetOrActionFound;
