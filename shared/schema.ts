...
// ==================== PaySpace Leave System ====================

export const leaveAuthModeEnum = pgEnum('leave_auth_mode', ['oauth', 'basic', 'token', 'soap']);
export const leaveRunStatusEnum = pgEnum('leave_run_status', ['running', 'success', 'partial', 'fail']);
export const leaveRunTriggerEnum = pgEnum('leave_run_trigger', ['schedule', 'manual']);
export const leaveEventStatusEnum = pgEnum('leave_event_status', ['approved', 'cancelled', 'other']);
export const leaveLedgerEventTypeEnum = pgEnum('leave_ledger_event_type', ['created', 'modified', 'cancelled', 'unknown']);
export const leaveLedgerImportStatusEnum = pgEnum('leave_ledger_import_status', ['pending', 'applied', 'failed', 'skipped']);

export const payspaceSettings = pgTable("payspace_settings", {
  id: serial("id").primaryKey(),
  isEnabled: boolean("is_enabled").notNull().default(false),
  companyCode: text("company_code"),
  apiBaseUrl: text("api_base_url"),
  authMode: leaveAuthModeEnum("auth_mode").notNull().default('oauth'),
  apiUsername: text("api_username"),
  apiPasswordEncrypted: text("api_password_encrypted"),
  apiTokenEncrypted: text("api_token_encrypted"),
  showLeaveType: boolean("show_leave_type").notNull().default(false),
  showFullSurname: boolean("show_full_surname").notNull().default(false),
  ...
  lookaheadDays: integer("lookahead_days").notNull().default(365),
  lastSyncAt: timestamp("last_sync_at"),
...
export const insertPayspaceSettingsSchema = createInsertSchema(payspaceSettings).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertPayspaceSettings = z.infer<typeof insertPayspaceSettingsSchema>;
export type PayspaceSettings = typeof payspaceSettings.$inferSelect;

export const leaveRuns = pgTable("leave_runs", {
  id: serial("id").primaryKey(),
  triggerType: leaveRunTriggerEnum("trigger_type").notNull(),
  startedAt: timestamp("started_at").notNull().defaultNow(),
  finishedAt: timestamp("finished_at"),
  status: leaveRunStatusEnum("status").notNull().default('running'),
  triggeredBy: text("triggered_by").notNull().default('system'),
  summaryJson: jsonb("summary_json"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
});

export const insertLeaveRunSchema = createInsertSchema(leaveRuns).omit({ id: true, startedAt: true, createdAt: true });
export type InsertLeaveRun = z.infer<typeof insertLeaveRunSchema>;
export type LeaveRun = typeof leaveRuns.$inferSelect;

export const leaveEvents = pgTable("leave_events", {
  id: serial("id").primaryKey(),
  externalLeaveId: text("external_leave_id").notNull().unique(),
  ...
  status: leaveEventStatusEnum("status").notNull().default('approved'),
...
export const insertLeaveEventSchema = createInsertSchema(leaveEvents).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertLeaveEvent = z.infer<typeof insertLeaveEventSchema>;
export type LeaveEvent = typeof leaveEvents.$inferSelect;

export const leaveLedger = pgTable("leave_ledger", {
  id: serial("id").primaryKey(),
  runId: integer("run_id").notNull().references(() => leaveRuns.id),
  externalLeaveId: text("external_leave_id").notNull(),
  eventType: leaveLedgerEventTypeEnum("event_type").notNull(),
  ...
  approvedBy: text("approved_by"),
  approvedAt: timestamp("approved_at"),
  importStatus: leaveLedgerImportStatusEnum("import_status").notNull().default('pending'),
  errorMessage: text("error_message"),
...
export const insertLeaveLedgerSchema = createInsertSchema(leaveLedger).omit({ id: true, detectedAt: true, createdAt: true });
