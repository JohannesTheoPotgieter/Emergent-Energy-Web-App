...
export const mytoolTasks = pgTable("mytool_tasks", {
  ...
  milestoneId: integer("milestone_id").references(() => mytoolTasks.id, { onDelete: "set null" }),
  blockedByDependencies: boolean("blocked_by_dependencies").notNull().default(false),
...
export const insertCounterpartySchema = createInsertSchema(counterparties).omit({ id: true, createdAt: true } as any);
