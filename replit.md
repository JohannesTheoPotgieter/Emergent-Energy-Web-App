...
-   **Portfolio Management**: Enhanced Gantt Chart with hierarchical milestones, two-layer progress bars, slippage warnings, and Cashflow tab showing aggregated FY26 weekly cashflow for all portfolio projects.
-   **Cashflow Page**: FY26 weekly cashflow with multi-select project filter (Popover + Command), KPI cards, trend chart, and detailed weekly grid. When projects are filtered, opening balance starts at zero (no manual overrides applied) so you can track just those projects' inflows and outflows. Backend supports comma-separated project filter.
-   **Project Detail Cashflow Tab**: FY26 weekly cashflow scoped to a single project with KPIs, chart, and table. Component: `CashflowTab.tsx`.
...
-   **Unified Work ("My Work")**: Consolidates personal and project-related tasks, calendar (Outlook integration), and communications. Aggregates tasks from ALL sources into a unified board: personal, operational, plan, engineering, quality, approvals, deliverables, TR register action items, MS 365 action items, and notifications. Includes source filter pill tabs, color-coded source badges, gradient KPI cards, List/Board view toggle, full New Task dialog, and enhanced task detail drawers. Calendar page sidebar also shows unified tasks with source filters. MS 365 and notification items are non-schedulable. Shared Task Tracking allows viewing tasks created by the user but assigned to others.
