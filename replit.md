...
-   **PO Generator**: Dialog-based Purchase Order generator accessible from project detail page. Creates PDFs matching Emergent Energy template (PO{seq}-{PROJECT}-{DATE}-{Supplier} naming). Supports multiple line items with duplicate/reorder/quick-add, supplier auto-fill from stored counterparty data, delivery instructions, and status tracking (draft/sent/approved/cancelled). PDF download on generation.
-   **Procurement / Counter Parties**: Supplier details panel in counterparty detail dialog. Stores VAT#, registration#, address, contact person/phone/email, bank details, payment terms, and notes. Data auto-fills into PO Generator when creating purchase orders. Collapsible view/edit mode with permission-gated editing.
