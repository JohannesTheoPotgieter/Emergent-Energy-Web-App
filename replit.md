...
-   **Data Merge**: `getProgramExpensesByProject` reads `normalized_cost_lines` as primary (preserving invoice/payment/COS data) and merges budget fields (`budgetTotal`, `budgetQty`, `budgetRateUnit`, `budgetCosTotal`, `forecastPaymentDate`) from `program_expense` by matching `rowNumber`.
