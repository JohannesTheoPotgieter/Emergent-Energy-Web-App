...
-   **Operating System Map**: Interactive 3-screen module built on top of the existing EE Info node infrastructure. Extended `ee_info_nodes` with `nodeType` (lifecycle_stage, department, process, step, tool, template, content), `departmentSlug`, `lifecycleStages` (jsonb array), `sortOrder`, `sopData` (structured SOP JSON), `parentNodeId`, `externalUrl`. Screens: Lifecycle Overview (`/ee-info/os`) shows timeline of 7 lifecycle stages with department chips and process cards; Department Drilldown (`/ee-info/os/department/:slug`) shows processes grouped by stage with search/filters; Process Detail (`/ee-info/os/process/:slug`) shows SOP (purpose, triggers, inputs, outputs, RACI, tools, templates, review cadence) with ordered steps; Templates Library (`/ee-info/os/templates`) is a searchable grid of template nodes linked to processes. COO can seed OS data, create process shells, create SOP shells, and edit nodes. 4 new permission entities: `ee_info_lifecycle`, `ee_info_departments`, `ee_info_processes`, `ee_info_templates`. All screens enforce permissions via `requirePermission` middleware and `PermissionGate` frontend. Mapping script at `POST /api/ee-info/os/seed` auto-categorizes existing process/tool/template nodes and creates lifecycle stage and department nodes (idempotent).
