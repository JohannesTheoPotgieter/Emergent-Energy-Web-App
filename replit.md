...
-   **Operating System Map**: Integrated directly into the `/ee-info` page, replacing the old Graph/Detail/Flow tabs. Three tabs remain: Operating System, Templates, Walkthroughs. The OS tab uses internal state-based navigation (no separate routes) for Lifecycle Overview (7 stages with expandable departments and process cards), Department Drilldown (processes grouped by stage with search/filters), and Process Detail (SOP with purpose, triggers, inputs, outputs, RACI, tools, templates, review cadence, and ordered steps). Extended `ee_info_nodes` with `nodeType` (lifecycle_stage, department, process, step, tool, template, content), `departmentSlug`, `lifecycleStages` (jsonb array), `sortOrder`, `sopData` (structured SOP JSON), `parentNodeId`, `externalUrl`. COO can seed OS data, create process shells, create SOP shells, and edit nodes. 4 permission entities: `ee_info_lifecycle`, `ee_info_departments`, `ee_info_processes`, `ee_info_templates`. Backend routes at `/api/ee-info/os/*`. Mapping script at `POST /api/ee-info/os/seed` auto-categorizes existing nodes (idempotent).
