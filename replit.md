...
- **Smart Import Legacy Parity**: The Smart Import commit writes ALL fields to `program_expense` that the legacy excelParser writes, including: `revenueAmount` (from revenue_recognition_amount), `actualCosTotal` (from actual_cos), `budgetCosTotal` (from budget_cos), `expenseQty` (from budget_qty), `expenseRateUnit` (from budget_rate), and `computedForecastPaymentDate` (from forecast_payment_date). Without these, COS tracker and cashflow calculations produce wrong numbers after re-import.
