...
-   **Execution Dashboard**: Focused on project plan tasks and project finance items only. Shows expandable project cards with plan completion (actual vs expected), revenue/cost tracking with progress bars, GP%, key dates (construction/commissioning/handover), and navigation links to project detail, plan tasks, revenue, and expenditure tabs. 5-column KPI summary strip: average completion, schedule health, revenue, costs, overall GP%. Project names are directly clickable links. Each expanded card section has inline "View" links. Responsive layout (1-col mobile, 2-col tablet, 4-col desktop).
...
-   **Plan Task work_items Sync**: Plan task edits (working-plan PATCH, planning-tasks PATCH) sync `percentComplete` to `work_items`. Uses two-phase lookup: first tries `legacyTable = 'project_plan'` + `legacyId`; if no match (e.g. Smart Import items with `legacyTable = 'normalized_plan_tasks'`), falls back to matching by `projectId + title + workstream='PM'`.
