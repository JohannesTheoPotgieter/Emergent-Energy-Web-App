...
- **Core Features**:
    ...
    -   **Project Management**: Operational Task Management (ClickUp-style task system with multiple views and detail drawers).
    ...
    -   **Phase Templates & Project Create**: Phase Template Admin (template CRUD with items, versioning, clone, activate, preview), Project Create (exec-only in-app project creation with auto-template application).
    -   **Smart Excel Import**: Smart Import Wizard (5-step review flow: Upload → Section Detection → Column Mapping → Issues & Anomalies → Preview & Commit), Normalized Project Views (Plan | Revenue | Expenditure tabs from normalized tables), Template Profile Learning (mapping rules persist and improve over imports), Counterparty Master Data (canonical names + aliases, auto-matching), Turnaround Time Metrics, Anomaly Detection (duplicate invoices, date order violations, missing amounts), Invoice Pattern Classification (auto-classifies expenditure lines by invoice number patterns to determine counterparty type with PREFIX/REGEX/TOKEN_SHAPE matching, confidence scoring, user review/override flow in Issues step).
    -   **Subcontractor Dashboard**: Aggregated view of installer/supplier accounts with KPI cards (biggest account, total counterparties, open amounts, upcoming 30d), filterable/sortable table, detail drawer with monthly spend trend chart, project breakdown, upcoming items, and all cost lines.
    ...
    -   **UX Guidance System**: Reusable additive overlay system (client/src/components/guidance/) with ActionBar (next best action + blocking/owner context), InlineTip (dismissable contextual hints with Why? expand), SmartValidation (what's wrong + how to fix + Fix button + Override with mandatory reason), MicroWalkthrough (first-time 2-3 step inline guide, localStorage-tracked, never repeats, replay button). Wired into Lifecycle Board, Engineering Tasks, Engineering Inbox, Quality Dashboard. Hooks in client/src/hooks/use-guidance.ts (useWalkthroughCompleted, useTipDismissed, useLastSelection).
    -   **Utilities & Admin**: SafeMoney Utilities (NaN-safe currency handling), My Tool (COO Execution Cockpit — Linear/Notion/Sunsama-style premium minimal interface with shared components: MyToolLayout, TaskCard, TaskDetailDrawer; DoD enforcement; natural language quick add; keyboard shortcuts ⌘K/⌘⏎; drag-drop email-to-task; time blocks; daily wrap; Read.ai Meeting Actions integration with webhook ingestion and action item conversion to tasks/priorities/projects).
