...
- **Canonical Revenue Recognition (POC method):** All Revenue KPIs across the app are computed from `normalized_cost_lines.revenue_recognition_amount`, gated on the underlying COS line being effectively realised (past-month auto-promote + canonical strict realisation check). The shared helpers live in `server/lib/finance/revenue-recognition.ts` (`recognitionAmountFor`, `sumRevenueRecognition[ByProject]`, `sumRealisedRevenueRecognition[ByProject]`, `getRevenueRecognitionForProject`); the realisation gate (`isPastMonthAutoRealised`, `isEffectivelyRealised`, `isEffectivelyCommitted`) is exported from `server/lib/finance/cos-realisation.ts`. Revenue Recognition (POC) is intentionally distinct from **Cashflow / Billing** (milestone-based: cash actually banked / invoices raised). POC consumers: `/api/overview` (`revenue_realised`, `revenue_planned`, `revenue_method:"POC"`), `/api/home/summary` (`financial.actualRevenue`, `financial.plannedRevenue`, `financial.revenueMethod`), `/api/projects/:name/health-summary` (`revenue.realisedPct`, `revenue.pocRevenueRealised`, `revenue.pocRevenuePlanned`), `/api/projects` per-project list (`actualRevenue`, `totalContractRevenue` with milestone fallback when POC is zero), `/api/financial-integration/...` (`cos_exceeds_revenue` warning uses POC with milestone fallback). Cashflow surfaces (`weeklyInflows`, `revenueOutstanding`, `monthlyRevOutstanding`, `totalPaidInflows`, project-header `inflowsRealisedPct`) intentionally REMAIN milestone-based — they answer "cash in the bank", not "revenue earned".
