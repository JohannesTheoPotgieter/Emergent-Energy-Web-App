...
- **PD Dashboard – Pipeline by Phase + Sign-Date Calendar (Task #77):** `/pd-dashboard` now renders a "Pipeline by phase" KPI card (per-canonical-phase opportunity count, total kWp, total deal value, with an in-row share-of-kWp bar) and an "Expected sign dates" month calendar directly above the existing PD operating board. Both share a single read-only fetch from `GET /api/pd/dashboard/pipeline-by-phase` (auth-gated: `requirePermission('pd_dashboard','view')`), which excludes won/lost, soft-deleted opportunities, and opportunities tied to soft-deleted clients. Phases are derived via `pdStageToLifecycle` (Pipedrive CRM stage → canonical S-code); unmapped stages bucket as `_UNSCOPED` so nothing silently disappears. The calendar is a dashboard-specific `SignDateCalendar` (one event per opportunity at `expected_close_date`, "no close date set" tray rendered above the grid; each chip displays client name, kWp and a phase code). Click-through opens the shared `OpportunityDrawer` from `/opportunities`. Both new sections use the existing `CollapsibleCard` (per-user open/closed state in localStorage). Integration test `qa/tests/integration/pd-dashboard-pipeline-by-phase.test.ts` is wired into `qa/release-gate.ts`. Pure additive — zero schema changes.
