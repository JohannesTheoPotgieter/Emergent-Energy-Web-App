...
| `expense_task_links.expenseId` (`expense_id`) | `storage`(12: `upsert/delete/updateExpenseTaskLink`), `routes/finance-legacy-extracted-routes`, `services/scheduler-commit`(8), `smart-import-routes`(9), **client** `cashflow.tsx` | 0 (count of populated `canonical_expense_id`) | **MIGRATE-FIRST** | `expense_task_links.canonicalExpenseId` (FK → `normalizedCostLines.id`) — the new column already exists and coexists per the schema migration comment | Every row's `canonical_expense_id` populated **and** all readers switched off `expense_id`. A parity layer already exists: `services/promoted-read-compat.ts` joins `legacy_program_expense_id` → `finance.cost_lines` and compares totals. |
...
| `server/routes/finance-legacy-extracted-routes.ts` | Registered in `routes/route-registry.ts`; referenced by `finance-inflows-repository`, `lib/finance/validators`, `lib/finance-trust/integrity-audit`; 9 routes in `qa/fixtures/route-coverage-baseline.json` | **MIGRATE-FIRST** | Re-home its endpoints under the current `server/routes/*.routes.ts` pattern (CLAUDE.md API style) | `qa/fixtures/route-coverage-baseline.json` — every covered route still answers after re-homing |
...
2. **`financial_reconciliation.tracker_vs_qb_*` columns** and **`expense_task_links.expenseId`** — small,
   bounded migrations with existing parity layers (R2 engine; `promoted-read-compat.ts` / `canonicalExpenseId`).
3. **`tracker_revenue_summary`, `writeback_*`, `QuickBooksReconciliationTab`, `reconciliation-qb-gap` COS path,
   `finance-legacy-extracted-routes.ts`** — feature-scoped migrations.
