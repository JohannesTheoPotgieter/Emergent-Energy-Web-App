...
3. **Failing smoke tests**
   - Command: `npm run test:smoke`
   - Block condition: non-zero exit code.

4. **Failing routes tests**
   - Command: `npm run test:routes`
   - Block condition: non-zero exit code.

5. **Failing workflow tests**
   - Command: `npm run test:workflows` (or override via `WORKFLOW_TEST_COMMAND`).
   - Block condition: non-zero exit code.

6. **Reconciliation proof missing or unstable**
...
7. **Critical route role proof missing**
...
8. **Critical defect file required and missing/failing**
...
- `npm run test:api`, `npm run test:smoke`, and `npm run test:workflows` automatically start/health-check/stop the local app server (`script/run-with-app.ts`) and inject a deterministic `SESSION_SECRET` when missing.
- Use `.env.test.example` as the baseline QA environment template in local/CI runs.
- Recommended single-command proof: `npm run qa:full-proof`.
