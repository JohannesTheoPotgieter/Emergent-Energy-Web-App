...
| `getProgramExpensesByProject` | 1190–1262 | `normalized_cost_lines`, `program_expense`, `project_info` | Same as above | No | YES: **6-field PE overlay** (no `adminDateOverride*`) + carry-forward from closed rows | `effectiveTo IS NULL` on NCL active; `effectiveTo IS NOT NULL` on NCL closed (for carry-forward) | YES | **DIFFERS from `_fetchAllProgramExpenses`**: overlays only 6 budget/forecast fields (lines 1251-1256), NOT the 4 `adminDateOverride*` fields. Also has carry-forward logic (lines 1200-1232). |
