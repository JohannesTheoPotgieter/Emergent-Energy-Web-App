...
| # | Check | Schema Verdict | Runtime Required? | Risk |
|---|---|---|---|---|
| 1.1 | Nullable `project_id` on financial tables | **STRUCTURAL GAP** | YES | MEDIUM |
| 1.2 | Missing ON DELETE CASCADE on financial FKs | Low risk (soft-delete used) | NO | LOW |
| 1.3 | Legacy `task_id` column with no FK | **STRUCTURAL GAP** | YES | MEDIUM |
| 2.1 | `work_items.created_by` nullable | **STRUCTURAL GAP** | YES | MEDIUM |
| 3 | 1:1 coverage (project → exec_state/settings) | Not enforced by schema | YES | HIGH |
| 4 | Orphaned work_item extensions | Protected by CASCADE | YES (safety) | LOW |
| 5.1 | `effective_from IS NULL` | Impossible (NOT NULL) | NO | PASS |
| 5.2 | `effective_to < effective_from` | **No CHECK constraint** | YES | MEDIUM |
| 6 | `organization_id` NULL | Impossible (NOT NULL DEFAULT 1) | NO | PASS |
| 6.1 | Financial tables missing direct org_id | By design (inherited via project) | NO | LOW |
| 7 | Dashboard metrics coverage | Not enforced by schema | YES | HIGH |
| 8.1 | Import snapshot on edited rows | **No CHECK constraint** | YES | HIGH |

### CRITICAL Items (require live database validation)

1. **1:1 coverage gaps** (Check 3) — If any `project_info` row lacks `project_execution_state` or `project_settings`, the app will likely crash or show incorrect data.
2. **Dashboard metrics gaps** (Check 7) — Missing metrics rows mean incomplete dashboards.
3. **Import snapshot gaps** (Check 8) — Edited rows without snapshots break import reconciliation.

### Recommended Structural Improvements

1. **Add CHECK constraint** on all temporal tables: `CHECK (effective_to IS NULL OR effective_to >= effective_from)`
2. **Add CHECK constraint** on import tables: `CHECK (source != 'imported_edited' OR import_snapshot IS NOT NULL)`
3. **Consider making `work_items.created_by` NOT NULL** for audit traceability.
4. **Consider making financial `project_id` columns NOT NULL** with backfill of existing NULL values.
5. **Clean up legacy `task_id` columns** on `task_comments`, `task_checklists`, `task_attachments`, `task_deliverables`, `task_activity_log`, `task_watchers` — these reference a dropped table.

---

*Report generated by static schema analysis. All runtime queries above must be executed against the live database to produce definitive pass/fail verdicts.*
