...
|---|---|---|---|
| 1 | Schema-to-DB table diff | 10 migration-only tables missing Drizzle definitions | WARNING |
| 2 | Duplicate table definitions | **PASS** — none found | — |
| 3 | Unused enums | **PASS** — all 59 enums referenced | — |
| 4 | FK index coverage | **12 CRITICAL/HIGH unindexed FKs** | HIGH |
| 5 | Remaining `project_name` columns | 44 tables — by design (import keys + snapshots) | INFO |

### Action Items

1. **HIGH:** Add indexes for the 12 critical/high unindexed FK columns (especially `work_item_assignments.work_item_id`, `work_item_dependencies.predecessor_id/successor_id`, `deliverable_versions.deliverable_id`, `qc_item_instance.checklist_id`)
2. **MEDIUM:** Add Drizzle schema definitions for the 10 migration-only tables if they are actively queried (`evidence_*`, `project_handover_*`, `project_pd_pm_handover`, `project_subcontractor_assignments`, `pm_compliance_tracking`, `project_linkage_review_queue`)
3. **LOW:** Consider removing `project_name` columns from tables that have `project_id` FK and don't need the import key (e.g., `dashboard_project_metrics`, `derived_project_kpis`)

---

*Report generated by static schema/migration analysis. Index coverage should be verified against the live database with `pg_indexes` for complete accuracy.*
