...
function generateEngineeringExcel(workbook: ExcelJS.Workbook, data: any, monthLabel: string) {
  ...
  const kpiRows = [
    ...
    { metric: "Cumulative Completion Rate %", value: kpis.cumulativeCompletionRate ? `${kpis.cumulativeCompletionRate.toFixed(1)}%` : "0%" },
    { metric: "Monthly Completion Rate %", value: kpis.monthlyCompletionRate ? `${kpis.monthlyCompletionRate.toFixed(1)}%` : "0%" },
