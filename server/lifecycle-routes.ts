  ...
  app.get("/api/lifecycle-board/execution-dashboard", requireAuth, async (_req: Request, res: Response) => {
    try {
      ...
      for (const project of activeProjects) {
        ...
        if (inflowRisk) {
          const inflowPct = plannedRevenueFy > 0 ? Math.round((openInflowFy / plannedRevenueFy) * 100) : 0;
          actionRows.push({ projectId: project.id, projectName: project.projectName, queue: "Inflow at Risk", issueTitle: `R${Math.round(openInflowFy).toLocaleString()} open of R${Math.round(plannedRevenueFy).toLocaleString()} planned (${inflowPct}% outstanding)`, severity: inflowPct > 60 ? "Critical" : "High", owner: project.pm || "Unassigned", dueDate: null, link: `/project/${encodeURIComponent(project.projectName)}?tab=revenue` });
        }
        if (outflowRisk) {
          const outflowPct = plannedExpenditureFy > 0 ? Math.round((openExpenditureFy / plannedExpenditureFy) * 100) : 0;
          actionRows.push({ projectId: project.id, projectName: project.projectName, queue: "Expenditure / COS at Risk", issueTitle: `R${Math.round(openExpenditureFy).toLocaleString()} open of R${Math.round(plannedExpenditureFy).toLocaleString()} planned (${outflowPct}% outstanding)`, severity: outflowPct > 60 ? "Critical" : "High", owner: project.pm || "Unassigned", dueDate: null, link: `/project/${encodeURIComponent(project.projectName)}?tab=expenditure` });
        }
        for (const t of engBlockers.slice(0, 5)) actionRows.push({ projectId: project.id, projectName: project.projectName, queue: "Engineering Bottlenecks", issueTitle: t.title || "Engineering blocker", severity: "High", owner: t.ownerUserId ? (usersById.get(t.ownerUserId) || "Owner") : "Unassigned", dueDate: t.dueDate || null, link: `/project/${encodeURIComponent(project.projectName)}?tab=plan` });
        for (const q of openQuality.slice(0, 5)) actionRows.push({ projectId: project.id, projectName: project.projectName, queue: "Quality Issues", issueTitle: q.title, severity: q.severity || "Medium", owner: q.ownerUserId ? (usersById.get(q.ownerUserId) || "Owner") : "Unassigned", dueDate: q.dueDate || null, link: `/project/${encodeURIComponent(project.projectName)}` });
        for (const a of projectApprovals.slice(0, 5)) actionRows.push({ projectId: project.id, projectName: project.projectName, queue: "Pending Approvals / Decisions", issueTitle: a.title || "Pending approval", severity: "Medium", owner: a.assignedApprover ? (usersById.get(a.assignedApprover) || "Approver") : "Unassigned", dueDate: a.dueDate ? new Date(a.dueDate).toISOString().slice(0, 10) : null, link: `/project/${encodeURIComponent(project.projectName)}` });
      ...
      const onScheduleCount = projectRows.filter((p) => (p.actualProgressPct ?? 0) >= (p.expectedProgressPct ?? 0) - 5).length;
