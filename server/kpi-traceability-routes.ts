...
export function registerKpiTraceabilityRoutes(app: Express) {
  app.get("/api/admin/kpi-traceability", requireAuth, requireAdmin, async (_req: Request, res: Response) => {
    ...
    const kpis = [
      { id: "revenue_planned", name: "Total Planned Revenue", currentValue: plannedRev, sourceTable: "project_revenue_summary", sourceFields: "planned_revenue", formula: "SUM(project_revenue_summary.planned_revenue)", apiEndpoint: "/api/revenue-summary", consumingComponent: "Dashboard SummaryCard, RevenueTrackerTab" },
      { id: "revenue_actual", name: "Total Actual Revenue", currentValue: actualRev, sourceTable: "project_revenue_summary", sourceFields: "actual_revenue", formula: "SUM(project_revenue_summary.actual_revenue)", apiEndpoint: "/api/revenue-summary", consumingComponent: "Dashboard SummaryCard, RevenueTrackerTab" },
      { id: "cos_budget", name: "Total Budget COS", currentValue: agg.cos.totalBudgetCos, sourceTable: "normalized_cost_lines", sourceFields: "budget_total", formula: "SUM(normalized_cost_lines.budget_total) WHERE effective_to IS NULL", apiEndpoint: "/api/expenses", consumingComponent: "COS Control, ExpenditureTab, Dashboard SummaryCard" },
      { id: "cos_actual", name: "Total Actual COS", currentValue: agg.cos.totalActualCos, sourceTable: "normalized_cost_lines", sourceFields: "amount_ex_vat", formula: "SUM(normalized_cost_lines.amount_ex_vat) WHERE effective_to IS NULL", apiEndpoint: "/api/expenses", consumingComponent: "COS Control, ExpenditureTab, Dashboard SummaryCard" },
      ...
      { id: "cashflow_revenue", name: "Total Cashflow Revenue", currentValue: agg.cashflow.totalCashflowRevenue, sourceTable: "cashflow_points", sourceFields: "value (series_name LIKE '%revenue%')", formula: "SUM(cashflow_points.value) WHERE series_name ILIKE '%revenue%'", apiEndpoint: "/api/cashflow", consumingComponent: "CashflowTab, CashflowForecast" },
      { id: "cashflow_expenditure", name: "Total Cashflow Expenditure", currentValue: agg.cashflow.totalCashflowExpenditure, sourceTable: "cashflow_points", sourceFields: "value (series_name LIKE '%expenditure%')", formula: "SUM(cashflow_points.value) WHERE series_name ILIKE '%expenditure%'", apiEndpoint: "/api/cashflow", consumingComponent: "CashflowTab, CashflowForecast" },
      ...
      { id: "eng_tasks_total", name: "Engineering Tasks Total", currentValue: engTotal, sourceTable: "work_items", sourceFields: "id, status", formula: "COUNT(work_items.*) WHERE workstream = 'ENG' AND deleted_at IS NULL", apiEndpoint: "/api/engineering-tasks", consumingComponent: "EngineeringDashboard, EngineeringTasksPage" },
      { id: "eng_tasks_complete", name: "Engineering Tasks Complete", currentValue: engComplete, sourceTable: "work_items", sourceFields: "status", formula: "COUNT(work_items.*) WHERE workstream = 'ENG' AND status IN ('COMPLETE','Done') AND deleted_at IS NULL", apiEndpoint: "/api/engineering-tasks", consumingComponent: "EngineeringDashboard, EngineeringTasksPage" },
      { id: "eng_progress_pct", name: "Engineering Progress %", currentValue: engTotal > 0 ? Math.round((engComplete / engTotal) * 10000) / 100 : 0, sourceTable: "work_items", sourceFields: "status", formula: "(COUNT(status='COMPLETE') / COUNT(*)) × 100 WHERE workstream = 'ENG'", apiEndpoint: "/api/engineering-tasks", consumingComponent: "EngineeringDashboard, Dashboard SummaryCard" },
      ...
      { id: "mywork_personal_tasks", name: "Personal Tasks Count", currentValue: agg.workItems.personalTaskCount, sourceTable: "work_items", sourceFields: "id", formula: "COUNT(work_items.*) WHERE workstream = 'PERSONAL'", apiEndpoint: "/api/mytool/tasks", consumingComponent: "MyWorkTasksPage, MyToolTodayPage" },
      ...
      { id: "inflow_in_bank", name: "Milestones In Bank", currentValue: agg.inflows.inBankCount, sourceTable: "normalized_revenue_lines", sourceFields: "paid_date, paid_date_confirmed", formula: "COUNT(normalized_revenue_lines.*) WHERE paid_date IS NOT NULL AND paid_date_confirmed = true", apiEndpoint: "/api/inflows/:project", consumingComponent: "RevenueTrackingTab, Dashboard" },
    ];
