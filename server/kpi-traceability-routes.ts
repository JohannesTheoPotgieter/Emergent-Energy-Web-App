...
export function registerKpiTraceabilityRoutes(app: Express) {
  app.get("/api/admin/kpi-traceability", requireAuth, requireAdmin, async (_req: Request, res: Response) => {
    ...
    const kpis = [
      ...
      { id: "cashflow_revenue", name: "Total Cashflow Revenue", currentValue: agg.cashflow.totalCashflowRevenue, sourceTable: "cashflow_points", sourceFields: "value (series_name LIKE '%revenue%')", formula: "SUM(cashflow_points.value) WHERE series_name ILIKE '%revenue%'", apiEndpoint: "/api/cashflow-2026", consumingComponent: "CashflowTab, CashflowForecast" },
      { id: "cashflow_expenditure", name: "Total Cashflow Expenditure", currentValue: agg.cashflow.totalCashflowExpenditure, sourceTable: "cashflow_points", sourceFields: "value (series_name LIKE '%expenditure%')", formula: "SUM(cashflow_points.value) WHERE series_name ILIKE '%expenditure%'", apiEndpoint: "/api/cashflow-2026", consumingComponent: "CashflowTab, CashflowForecast" },
      ...
      { id: "inflow_total_value", name: "Total Milestone/Inflow Value", currentValue: agg.inflows.totalMilestoneValue, sourceTable: "normalized_revenue_lines", sourceFields: "amount_ex_vat", formula: "SUM(normalized_revenue_lines.amount_ex_vat) WHERE effective_to IS NULL", apiEndpoint: "/api/cashflow-2026/detail", consumingComponent: "RevenueTrackingTab, CashflowTab" },
      { id: "inflow_in_bank", name: "Milestones In Bank", currentValue: agg.inflows.inBankCount, sourceTable: "normalized_revenue_lines", sourceFields: "paid_date, paid_date_confirmed", formula: "COUNT(normalized_revenue_lines.*) WHERE paid_date IS NOT NULL AND paid_date_confirmed = true", apiEndpoint: "/api/cashflow-2026/detail", consumingComponent: "RevenueTrackingTab, Dashboard" },
    ];
