  ...
  app.get("/api/admin/kpi-traceability", requireAuth, requireAdmin, async (_req: Request, res: Response) => {
    try {
      const [revSummary, cosAgg, cashflowAgg, projCounts, planProgress, engAgg, qcAgg, opCount, ptCount, wiCount, pCount, inflowAgg] = await Promise.all([
        ...
        db.execute(sql`SELECT COUNT(*) as c FROM work_items WHERE deleted_at IS NULL AND legacy_table = 'operational_tasks'`).then(rows0).catch(() => ({ c: 0 })),
        db.execute(sql`SELECT COUNT(*) as c FROM work_items WHERE deleted_at IS NULL AND workstream = 'PERSONAL'`).then(rows0).catch(() => ({ c: 0 })),
      ...
      const kpis = [
        ...
        { id: "mywork_operational_tasks", name: "Operational Tasks Count (via work_items)", currentValue: Number(opCount?.c ?? 0), sourceTable: "work_items", sourceFields: "id", formula: "COUNT(work_items.*) WHERE legacy_table = 'operational_tasks'", apiEndpoint: "/api/work-items", consumingComponent: "MyWorkTasksPage, MyToolTodayPage" },
