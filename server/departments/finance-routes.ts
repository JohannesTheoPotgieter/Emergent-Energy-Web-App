...
const deleteOpeningBalanceSchema = z
  .object({
    weekStartDate: isoDateStr,
  })
  .strict();

const deleteOpexWeeklySchema = z
  .object({
    weekStartDate: isoDateStr,
  })
  .strict();

const trackerMonthlyBodySchema = z
  .object({
...
const cosToggleRealisedSchema = z
  .object({
    realised: z.boolean(),
...
const cosOverrideStatusSchema = z
  .object({
  ...
  .refine(
    (data) =>
      data.cosStatus === null || (data.reason !== undefined && data.reason.trim().length >= 3),
    { message: 'reason is required (min 3 chars) when cosStatus is not null' },
  );

const noRevenueLinkedSchema = z
  .object({
...
const revenueTabCostedSchema = z
  .object({
    revenue: decimalLike,
...
const revenueTabLinkTaskSchema = z
  .object({
...
const revenueTabDateOverrideSchema = z
  .object({
    milestoneRowNumber: z.number().int().min(0),
...
const expenditureOverridesSchema = z
  .object({
    overrides: z
      .array(
        z
...
const expenseTaskLinkDateOverrideSchema = z
  .object({
    dateOverride: isoDateStr,
    reason: z.string().max(1000).optional(),
  })
...
const addExpenseCategorySchema = z
  .object({
    projectName: z.string().min(1),
...
const insertTaskAsLineSchema = z
  .object({
    projectName: z.string().min(1),
    taskId: z.number().int(),
    expenseCategory: z.string().min(1).max(255),
...
import {
  QuickBooksLinksRepository,
  type QbLinkRef,
} from '../repositories/quickbooks-links-repository';
import {
...
async function getHighRiskProjectCostReadRows(
  projectName: string,
  projectIdParam?: number | null,
): Promise<any[]> {
...
function requireAdminOrFinancialEditor(req: Request, res: Response, next: NextFunction) {
  ...
  res
    ...
    .json({
      ...
      message: 'Admin or financial editor access required',
      code: 'ADMIN_REQUIRED',
    });
}
...
function isEffectivelyRealised(
  exp: any,
...
function isEffectivelyCommitted(
  exp: any,
  monthKey: string | null,
  currentMonthKey: string,
): boolean {
 ...
 */
function setFinanceTrustHeaders(res: Response, params: FinanceTrustHeaderParams) {
...
function resolveInflowEffectiveDates(
  ...
  if (taskLinks.length === 0) {
    return inflows.map((inf) => ({
      ...inf,
      effectiveDate:
        inf.adminDateOverride ||
        inf.paymentReceivedDate ||
        inf.computedForecastReceiptDate ||
        inf.plannedPaymentDate ||
        null,
    }));
  }
  ...
  for (const link of taskLinks) {
    ...
    normalizedLinkMap.set(
      `${normalizeProjectName(link.projectName)}::${link.milestoneRowNumber}`,
      link,
    );
...
function toRecentChangeSummary(
  changeRows: any[],
  fieldRowsByChangeSet: Map<number, any[]>,
  userNameById: Map<number, string>,
) {
...
function buildRevenueFieldAudit(
  ...
  const fieldChange = latestChange?.fieldChanges?.find(
  ...
  return {
    ...
    changedAt:
      overrideRecord?.updatedAt || overrideRecord?.createdAt || latestChange?.createdAt || null,
    changedByUserId: overrideRecord?.createdBy || latestChange?.actorUserId || null,
    changedByName: overrideRecord?.createdBy
      ? userNameById.get(overrideRecord.createdBy) || null
      : formatChangeUserName(latestChange?.actorUserId, latestChange?.actorRole, userNameById),
...
async function loadProjectFinanceGovernanceContext(
  ...
  type MicrosoftRow = Pick<
    ...
    | 'isRead'
    | 'linkedTaskId'
    | 'receivedOrStartDatetime'
  >;
  const [approvalRows, editRequestRows, changeRows, microsoftRows]: [
    ApprovalRow[],
    Awaited<
      ReturnType<typeof financialIntegrationRepository.listEditRequestsForProjectWithRequester>
    >,
    ChangeRow[],
    MicrosoftRow[],
  ] = await Promise.all([
  ...
  const userNameById = new Map<number, string>(
    userRows.map((row: { id: number; name: string | null }) => [
      row.id,
  ...
  return {
    ...
    editRequests: {
      ...
      pending: pendingEditRequests.slice(0, 5).map((row) => ({
        ...
        requestedByName:
          row.requestedByName ||
          (row.requestedByUserId != null ? userNameById.get(row.requestedByUserId) : null) ||
          null,
  ...
  try {
    ...
    for (const exp of filtered) {
      ...
      if (
        ...
        invoiceDate >= filterStart &&
        invoiceDate <= filterEnd &&
      ...
      if (
        ...
        const daysSinceInvoice = Math.floor(
          (today.getTime() - invoiceDateObj.getTime()) / (1000 * 60 * 60 * 24),
        );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const allInflows = resolveInflowEffectiveDates(
        rawInflows,
        allTaskLinks,
        allOpTasks,
        allPlanTasks,
      ...
      const manualMap = new Map(
        manualBalances.map((m) => [m.weekStartDate, parseFloat(m.openingBalance || '0')]),
      );
      const opexMonthlyMap = new Map(
        opexBudgets.map((o) => [o.monthKey, parseFloat(o.amount || '0')]),
      );
      ...
      const addScopeDateCandidate = (dateValue: unknown) => {
        const iso = String(dateValue ?? '').slice(0, 10);
        if (/^\d{4}-\d{2}-\d{2}$/.test(iso)) scopeDateCandidates.push(iso);
      };
      for (const inflow of allInflows) addScopeDateCandidate(inflow.effectiveDate);
      for (const expense of allExpenses)
        addScopeDateCandidate(getExpenseEffectiveDateAndSource(expense).date);
      for (const manual of manualBalances) addScopeDateCandidate(manual.weekStartDate);
      for (const opex of opexBudgets) addScopeDateCandidate(`${opex.monthKey}-01`);
      ...
      const fyEnd = new Date(
        Date.UTC(scopeEndDate.getUTCFullYear(), scopeEndDate.getUTCMonth() + 1, 0),
      );
      ...
      for (const e of itemExpenses) {
        ...
        const hasAmt =
          rawAmt != null && rawAmt !== '' && Number.isFinite(parseFloat(String(rawAmt)));
        const hasInvoice = !!(
          (e as any).expenseInvoiceNumber && String((e as any).expenseInvoiceNumber).trim()
        );
      ...
      for (const inf of allInflows) {
        ...
        const hasAmt =
          rawAmt != null && rawAmt !== '' && Number.isFinite(parseFloat(String(rawAmt)));
        const hasInvoice = !!(
          (inf as any).invoiceNumber && String((inf as any).invoiceNumber).trim()
        );
        if (!hasAmt && hasInvoice) cashflowNullCount += 1;
      }
      const weeks: any[] = [];
        ...
        for (const expense of itemExpenses) {
          ...
          if (d >= weekStart && d < weekEnd) {
            ...
            const payDateConfirmed =
              expense.expensePaymentDate &&
              isDateConfirmed(
                (expense as any).paymentDateConfirmed,
                (expense as any).paymentDateFontColor,
              );
      ...
      const forecastOutflowsYtd = weeks.reduce(
        (s: number, w: any) => s + (w.forecastOutflows || 0),
        0,
      );
      diagnostics.actualOutflowsYtd = actualOutflowsYtd;
      diagnostics.forecastOutflowsYtd = forecastOutflowsYtd;
      console.log(
        `[Cashflow2026] Expenses: ${allExpenses.length} total (${normalizedCount} normalized, ${legacyCount} legacy), ${itemCount} items. Outflows YTD: ${totalOutflowsYtd.toFixed(0)} (actual ${actualOutflowsYtd.toFixed(0)}, forecast ${forecastOutflowsYtd.toFixed(0)})`,
      );

      const summaryLastImportDate = itemExpenses.reduce<string | null>((max, e: any) => {
        const at =
          e.snapshotRunCommittedAt || e.createdAt
            ? new Date(e.snapshotRunCommittedAt ?? e.createdAt).toISOString()
            : null;
        if (!at) return max;
        return max === null || at > max ? at : max;
      }, null);
      const summaryMissingTermsCount = itemExpenses.filter((e: any) =>
        paymentTermsMissing(e),
      ).length;
      ...
      const quickBooksLinkStatus: 'linked' | 'partial' | 'unmatched' | 'unknown' =
        !qbConnectionStatus
          ? 'unknown'
          : qbConnectionStatus.health === 'healthy' && !qbConnectionStatus.isStale
            ? 'linked'
            : qbConnectionStatus.health === 'failing'
              ? 'unmatched'
              : qbConnectionStatus.health === 'stale' || qbConnectionStatus.isStale
                ? 'partial'
                : 'unknown';
      ...
      const overrideInEffect = weeks.some(
        (w: any) =>
          w.hasManualOverride === true ||
          w.hasOpexOverride === true ||
          w.hasAvailPayOverride === true,
      );
  ...
  async (req, res) => {
    try {
      ...
      if (!weekStart || !/^\d{4}-\d{2}-\d{2}$/.test(weekStart)) {
        return res
          .status(400)
          .json({ error: 'Invalid week parameter', message: 'Provide ?week=YYYY-MM-DD' });
      }
      const projectFilterRaw = req.query.project ? String(req.query.project) : null;
      const projectFilters: Set<string> | null = projectFilterRaw
        ? new Set(
      ...
      const resolvedInflows = resolveInflowEffectiveDates(
        rawInflows,
        allTaskLinks,
        allOpTasks,
        allPlanTasks,
      ...
      const outflows = allExpenses
        ...
        .map((e) => {
          ...
          const originalDate =
            e.expensePaymentDate ||
            (e as any).computedForecastPaymentDate ||
            (e as any).forecastPaymentDate ||
            (e as any).expenseInvoicedDate ||
            null;
          ...
          return {
            ...
            lastImportedAt:
              (e as any).snapshotRunCommittedAt || (e as any).createdAt
            ...
            forecastDateShiftDays: computeDateShiftDays(
              ((e as any).importSnapshot as Record<string, unknown> | null)?.forecastPaymentDate,
              (e as any).forecastPaymentDate,
            ),
          };
        });
      ...
      const inflows = resolvedInflows
        ...
        .map((inf: any) => {
          ...
          return {
            ...
            lastImportedAt:
              (inf as any).snapshotRunCommittedAt || (inf as any).createdAt
                ? new Date(
                    (inf as any).snapshotRunCommittedAt ?? (inf as any).createdAt,
                  ).toISOString()
                : null,
      ...
      try {
        ...
        billCandidates = (billsRaw?.QueryResponse?.Bill ?? [])
        ...
        invoiceCandidates = (invoicesRaw?.QueryResponse?.Invoice ?? [])
          .map(invoiceRawToSummary)
          .map((inv: any) => ({
            id: String(inv.id),
        ...
        const outflowIds = outflows
          .map((o: any) => Number(o.expenseId))
          .filter((id: number) => Number.isFinite(id));
        const inflowIds = inflows
          .map((i: any) => Number(i.inflowId))
          .filter((id: number) => Number.isFinite(id));
      ...
      const enrichedOutflows = outflows.map((row: any) => {
        ...
        const qbStatus = deriveOutflowsQbStatus(
          match.matched?.balance ?? null,
          !!match.matched,
          row.expenseActualTotal ?? null,
        );
        const qbUncertain =
          qbStatus !== 'Unknown' && (qbIsStale || match.qbMatchConfidence === 'low');
      ...
      const enrichedInflows = inflows.map((row: any) => {
        ...
        const qbStatus = deriveInflowsQbStatus(
          match.matched?.balance ?? null,
          !!match.matched,
      ...
      const uncertainCount = [...enrichedOutflows, ...enrichedInflows].filter(
        (r: any) => !!r.qbUncertain,
      ).length;
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const row = await financeExpenseRepository.updateCostLineAdminDateOverride(
        expenseId,
        overrideFields,
      );
      if (!row) {
      ...
      if (dateOverride) {
        ...
        if (existingId === null) {
          await manualEditFlagsRepository.createProtectedFlag({
            ...flagKey,
            editedByUserId: userId,
            editedAt: now,
          });
        } else {
          await manualEditFlagsRepository.refreshProtectedFlag(existingId, {
            editedByUserId: userId,
            editedAt: now,
          });
        }
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const row = await financeInflowsRepository.updateRevenueLineAdminDateOverride(
        inflowId,
        overrideFields,
      );
      if (!row) {
...
router.post(
  '/api/tracker-monthly',
  requireAuth,
  requireTrackerPermission('edit'),
  validateBody(trackerMonthlyBodySchema),
  async (req, res) => {
    try {
...
router.get(
  '/api/tracker-monthly/:type',
  requireAuth,
  requireTrackerPermission('view'),
  async (req, res) => {
    try {
...
router.get('/api/cos-tracker', requireAuth, requirePermission('cos', 'view'), async (req, res) => {
  try {
    const fyScope = resolveFinanceYearScope(req.query);
    const qbStartDate = fyScope.startDate ?? '2000-01-01';
    const qbEndDate = fyScope.endDate ?? '2100-12-31';
    const [manualEntries, rawCostLines, links, pnlReport] = await Promise.all([
      ...
      getMonthlyPnLReport(qbStartDate, qbEndDate).catch(() => null),
    ]);
    ...
    for (const detail of qbCosAccountDetails) {
      ...
      const label = `${detail.accountName || 'QuickBooks account'}${detail.accountId ? ` (${detail.accountId})` : ''}`;
      if (!qbCosAccountProjectsByMonth.has(detail.monthKey))
        qbCosAccountProjectsByMonth.set(detail.monthKey, new Map());
      const bucket = qbCosAccountProjectsByMonth.get(detail.monthKey)!;
      bucket.set(label, (bucket.get(label) ?? 0) + detail.amount);
    }
    ...
    const appOnlyPendingByMonth = new Map<
      string,
    ...
    if (
      ...
      console.warn(
        '[finance-trust][cos-tracker] realised COS is zero despite invoice-bearing cost lines; check invoiceDateConfirmed and month-bucketing fields.',
      );
    }
    ...
    const sourceMonthKeys = [
      ...
      ...plannedByMonth.keys(),
      ...appOnlyPendingByMonth.keys(),
      ...qbCosByMonth.keys(),
      ...qbCosAccountProjectsByMonth.keys(),
    ];
    ...
    for (const monthKey of monthKeys) {
      const [yr, monthNumber] = monthKey.split('-').map(Number);
      const monthDate = new Date(Date.UTC(yr, monthNumber - 1, 1));
      ...
      months.push({
        ...
        qbOnlyProjects: mapToSortedArray(qbCosAccountProjectsByMonth.get(monthKey) ?? new Map()),
...
router.get(
  '/api/cos-tracker/project/:projectName',
  requireAuth,
  ...
  async (req, res) => {
    try {
      ...
      if (fromMonthKey) {
        ...
        if (fromMonthKey > monthKey)
          return res.status(400).json({ error: 'fromMonthKey must be <= monthKey' });
      }
      ...
      const [allCostLines, links, rawBills] = await Promise.all([
        ...
        getBills(monthStart, monthEnd).catch((err) => {
          console.warn(
            '[cos-month-detail] getBills failed — continuing with app-only data:',
      ...
      for (const row of allCostLines) {
        ...
        const reasonBucket: LineItem['reasonBucket'] =
          cosState === 'realised'
            ...
            : cosState === 'committed'
              ? 'matched committed'
              : hasInvoice && !invoiceDateConfirmed
                ? 'app-only pending'
                : 'planned';
        ...
        if (
          stateFilterNorm === 'unrealised' &&
          !(cosState === 'planned' || cosState === 'committed')
        )
          continue;
      ...
      const includeQbOnly =
        !stateFilterNorm || stateFilterNorm === 'all' || stateFilterNorm === 'qb_actual';
      if (includeQbOnly) {
      ...
      const realisedTotal = items
        .filter((i) => i.cosState === 'realised')
        .reduce((s, i) => s + (i.appAmount ?? 0), 0);
      const committedTotal = items
        .filter((i) => i.cosState === 'committed')
        .reduce((s, i) => s + (i.appAmount ?? 0), 0);
      const plannedTotal = items
        .filter((i) => i.cosState === 'planned')
        .reduce((s, i) => s + (i.appAmount ?? 0), 0);
      const qbOnlyTotal = items
        .filter((i) => i.cosState === 'qb_actual')
      ...
      const expectedTotal =
        stateFilterNorm === 'realised'
          ? realisedTotal
          : stateFilterNorm === 'committed'
            ? committedTotal
            : stateFilterNorm === 'planned'
              ? plannedTotal
              : stateFilterNorm === 'unrealised'
                ? unrealisedTotal
                : stateFilterNorm === 'qb_actual'
                  ? qbOnlyTotal
                  : stateFilterNorm === 'recognised'
                    ? recognisedTotal
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const ignoreMeta = new Map<
        string,
        { reason: string; ignoredByName: string | null; ignoredAt: string }
      >();
      ...
      for (const bill of bills) {
        for (const lr of billRawToLineRows(bill)) {
          ...
          else if (resolution.projectName)
            bucket = 'tracker_gap'; // fuzzy-resolved rows fold in here; UI shows the strategy badge so finance can confirm
      ...
      const byBucket: Record<
        GapBucket,
        { count: number; amount: number; openCount: number; openAmount: number }
      > = {
      ...
      const byProject = new Map<
        string,
        { project: string; count: number; openCount: number; amount: number; openAmount: number }
      >();
      for (const r of rows) {
        if (r.bucket !== 'tracker_gap') continue;
        const key = r.resolvedProjectName ?? '(unknown)';
        if (!byProject.has(key))
          byProject.set(key, { project: key, count: 0, openCount: 0, amount: 0, openAmount: 0 });
      ...
      const unmappedClassMap = new Map<
        string,
        {
          classRefName: string;
          count: number;
          amount: number;
          sampleVendors: Set<string>;
          sampleCustomers: Set<string>;
        }
      >();
      for (const r of rows) {
        if (r.bucket !== 'unmapped_class' || !r.classRefName) continue;
        if (!unmappedClassMap.has(r.classRefName))
          unmappedClassMap.set(r.classRefName, {
            classRefName: r.classRefName,
            count: 0,
            amount: 0,
            sampleVendors: new Set(),
...
router.post(
  '/api/cos-tracker/tracker-gap/ignore',
  requireAuth,
  requirePermission('cos', 'edit'),
...
router.delete(
  '/api/cos-tracker/tracker-gap/ignore/:id',
  requireAuth,
  requirePermission('cos', 'edit'),
...
router.post(
  '/api/cos-tracker/tracker-gap/class-override',
  requireAuth,
  requirePermission('cos', 'edit'),
      ...
      if (code === '23505') {
        ...
        return res
          ...
          .json({
            ...
            detail: 'Another mapping was just saved for this class. Refresh and retry.',
          });
      }
...
router.delete(
  '/api/cos-tracker/tracker-gap/class-override/:id',
  requireAuth,
  requirePermission('cos', 'edit'),
  validateBody(overrideUndoBodySchema),
  async (req, res) => {
    try {
      const id = Number(req.params.id);
      if (!Number.isFinite(id)) return res.status(400).json({ error: 'invalid_id' });
      ...
      await logAuditFromReq(req, {
        ...
        changesJson: {
          previous_state: {
            classRefName: prev.classRefName,
...
async function getProjectRevenueLinesConsistent(projectName: string): Promise<any[]> {
  ...
  const normalizedTarget = (projectName || '')
    .replace(/_Tracker$/i, '')
    .trim()
    .toLowerCase();
  return allInflows.filter((r: any) => {
    const rName = ((r.projectName as string) || '')
      .replace(/_Tracker$/i, '')
      .trim()
      .toLowerCase();
    ...
    try {
      ...
      for (const inv of invoices) {
        for (const lr of invoiceRawToLineRows(inv)) {
          ...
          if (resolution.projectName) {
            const candidates =
              revLinesByProjectKey.get(normalizeProjectKey(resolution.projectName)) ?? [];
            const target = lr.lineAmountExVat ?? 0;
      ...
      const unmappedCustomerMap = new Map<
        string,
        {
          customerName: string;
          count: number;
          amount: number;
          sampleClasses: Set<string>;
      ...
      for (const r of rows) {
        ...
        if (!unmappedCustomerMap.has(r.customerName))
          unmappedCustomerMap.set(r.customerName, {
      ...
      const unmappedCustomerList = [...unmappedCustomerMap.values()]
        .map((v) => ({
          ...
          suggestions: rankRevenueProjectSuggestions({
            ...
            priorOverridesForCustomer:
              overridesByCustomerHistory.get(v.customerName.toLowerCase().trim()) ?? [],
          }),
        }))
        .sort((a, b) => b.amount - a.amount);
    ...
    try {
      ...
      if (code === '23505') {
        return res
          ...
          .json({
            error: 'concurrent_update',
            detail: 'Another mapping was just saved for this customer. Refresh and retry.',
          });
      }
  ...
  async (req, res) => {
    try {
      ...
      await logAuditFromReq(req, {
        ...
        changesJson: {
          previous_state: {
            customerRefName: prev.customerRefName,
            projectName: prev.projectName,
            note: prev.note,
          },
          new_state: null,
...
router.get(
  '/api/cos-tracker/reconciliation',
  requireAuth,
  requirePermission('cos', 'view'),
  async (req, res) => {
    try {
      ...
      const reasonCodesFor = (row: any, bill: any) => {
        ...
        const rowSupplier = String(row?.counterpartyName || '')
          .trim()
          .toLowerCase();
        const billSupplier = String(bill?.vendorName || '')
          .trim()
          .toLowerCase();
...
async function updateExpenseFieldsDualTable(
  ...
  console.warn(
    `[deprecation] updateExpenseFieldsDualTable called with legacy PE id=${id}. PE writes are deprecated.`,
  );
  return storage.updateProgramExpenseFields(id, fields, expectedUpdatedAt);
}
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const { realised, expectedUpdatedAt } = req.body as {
        realised: boolean;
        expectedUpdatedAt?: string;
      };
      if (typeof realised !== 'boolean')
        return res.status(400).json({ error: 'realised (boolean) required' });
      ...
      const updated = await updateExpenseFieldsDualTable(
        id,
        {
  ...
  async (req, res) => {
    try {
      ...
      const {
        cosStatus,
        invoiceDate,
        invoiceDateConfirmed: invoiceDateConfirmedOverride,
        reason,
        expectedUpdatedAt,
      } = req.body as {
      ...
      if (!validStatuses.includes(cosStatus)) {
        return res
          ...
          .json({
            error:
              "cosStatus must be 'Planned', 'Committed', 'COS Realised', or null (to clear override)",
          });
      }
      ...
      if (cosStatus !== null && (!reason || !reason.trim())) {
        return res.status(400).json({
          ...
          message:
            'A reason is required when setting a COS override. This is a financial control — the audit trail must explain why recognition is diverging from the linked invoice evidence.',
        });
      }
...
router.post(
  '/api/cos-periods/:yyyyMm/lock',
  requireAuth,
  requirePeriodLockRole,
  validateBody(cosPeriodLockSchema),
  async (req, res) => {
    try {
    ...
    try {
      ...
      if (!reason) {
        return res.status(400).json({
          ...
          message:
            'A reason is required to unlock a COS period. This is a financial control — the audit trail must explain why the month is being reopened.',
        });
      }
...
router.patch(
  '/api/cost-lines/:id/no-revenue-linked',
  requireAuth,
  requireAdmin,
  validateBody(noRevenueLinkedSchema),
  async (req, res) => {
    try {
      const id = parseIntParam(req.params.id);
      if (isNaN(id)) return res.status(400).json({ error: 'Invalid cost line id' });
      const { noRevenueLinked, expectedUpdatedAt } = req.body as {
...
router.get(
  '/api/revenue-tracker/project/:projectName',
  requireAuth,
  requirePermission('revenue_tracker', 'view'),
  async (req, res) => {
    try {
...
router.get(
  '/api/gp-tracker',
  requireAuth,
  requirePermission('gp_tracker', 'view'),
  async (req, res) => {
    try {
      const [allExpenses, allInflowsRaw, revManualEntries, cosManualEntries, cosOverrideMap] =
        await Promise.all([
      ...
      const revManualBudgetMap = new Map(
        revManualEntries.map((e) => [e.monthKey, e.budget ? parseFloat(e.budget) : 0]),
      );
      const cosManualBudgetMap = new Map(
      ...
      for (const rev of allInflowsRaw) {
        ...
        const hasAmt =
          rawAmt != null && rawAmt !== '' && Number.isFinite(parseFloat(String(rawAmt)));
        const hasInvoice = !!(
          (rev as any).invoiceNumber && String((rev as any).invoiceNumber).trim()
        );
      ...
      for (const exp of allExpenses) {
        ...
        const hasAmt =
          rawAmt != null && rawAmt !== '' && Number.isFinite(parseFloat(String(rawAmt)));
        const hasInvoice = !!(
          (exp as any).expenseInvoiceNumber && String((exp as any).expenseInvoiceNumber).trim()
        );
        if (!hasAmt && hasInvoice) gpNullCount += 1;
      }
      ...
      res.json({
        ...
        overallGpPct,
        ytdGP: finalYtdGP,
        ytdBudget: finalYtdBudget,
        ytdVariance: finalYtdVariance,
        ytdGpPct: finalYtdGpPct,
        nullCount: gpNullCount,
      });
    } catch (error) {
...
router.get(
  '/api/gp-tracker/project/:projectName',
  requireAuth,
  requirePermission('gp_tracker', 'view'),
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      const {
        ...
        project,
        state: stateFilter,
      } = req.query as { monthKey?: string; project?: string; state?: string };
      if (!monthKey) return res.status(400).json({ error: 'monthKey required' });
...
async function revenueTrackerHandler(req: Request, res: Response) {
  try {
    ...
    const [revenueLinks, qbInvoicesRaw, qbMonthlyPnL] = await Promise.all([
      ...
      getInvoices(qbStartDate, qbEndDate).catch(() => ({ QueryResponse: { Invoice: [] } })),
      getMonthlyPnLReport(qbStartDate, qbEndDate).catch(() => null),
    ]);
    ...
    for (const inflow of allInflowsRaw) {
      ...
      const hasInvoice = !!(
        (inflow as any).invoiceNumber && String((inflow as any).invoiceNumber).trim()
      );
    ...
    for (const detail of qbRevenueAccountDetails) {
      if (!Number.isFinite(detail.amount) || detail.amount === 0) continue;
      if (!monthKeyInFinanceScope(detail.monthKey, fyScope)) continue;
      const label = `${detail.accountName || 'QuickBooks account'}${detail.accountId ? ` (${detail.accountId})` : ''}`;
      if (!qbRevenueByMonth.has(detail.monthKey))
        qbRevenueByMonth.set(detail.monthKey, { total: 0, projects: new Map() });
      const bucket = qbRevenueByMonth.get(detail.monthKey)!;
    ...
    for (const monthKey of monthKeys) {
      ...
      const qbVsAppVariancePct =
        qbRevenueActual !== 0 ? (qbVsAppVariance / qbRevenueActual) * 100 : 0;
...
router.get(
  '/api/revenue-tracker/month-detail',
  requireAuth,
  requirePermission('revenue_tracker', 'view'),
  async (req, res) => {
    try {
      ...
      const [allExpenses, cosOverrideMapRMD] = await Promise.all([
        project
          ? getCanonicalProjectCostLinesByName(project).then((r) => r.rows)
          : getCanonicalAllCurrentCostLines(),
        Promise.resolve(new Map()),
      ]);
      const [revenueLinks, qbInvoicesRaw] = await Promise.all([
        qbLinksRepository.listActiveLinksByPair('revenue_line', 'invoice'),
        getInvoices(`${monthKey}-01`, monthEndRev).catch(() => ({
...
router.get(
  '/api/revenue-tracker/reconciliation',
  requireAuth,
  requirePermission('revenue_tracker', 'view'),
  async (req, res) => {
    try {
      ...
      for (const row of recRows) {
        ...
        if (row.matchStatus === 'app_only') s.appOnlyItemCount++;
        if (row.matchStatus === 'qb_only') s.qbOnlyItemCount++;
        if (row.reasonCodes.includes('Missing payment receipt date'))
          s.missingPaymentReceiptDateCount++;
        if (row.reasonCodes.includes('Missing invoice number')) s.missingInvoiceCount++;
        if (
          row.reasonCodes.includes('Project mapping missing') ||
          row.reasonCodes.includes('Customer mapping missing')
        )
          s.mappingIssueCount++;
      }
    ...
    try {
      ...
      if (startDate && typeof startDate === 'string') {
        expenses = expenses.filter(
          (e) => e.expensePaymentDate && e.expensePaymentDate >= startDate,
        );
      }
...
router.get(
  '/api/projects/:projectName/cost-lines',
  requireAuth,
  requirePermission('cashflow', 'view'),
  async (req, res) => {
    try {
...
router.get(
  '/api/projects/:projectName/revenue-lines',
  requireAuth,
  requirePermission('cashflow', 'view'),
  async (req, res) => {
    try {
    ...
    try {
      ...
      if (!projectName) {
        return res.status(400).json({
          ...
          message:
            'Please select a specific project to view cashflow data. The full portfolio cashflow is available in the Cashflow 2026 view.',
      ...
      if (weekDates.length > 0) {
        const projectNames = projectName
          ? [projectName]
          : [...new Set(points.map((p) => p.projectName))];

        const dynamicPoints: any[] = [];

        for (const pn of projectNames) {
          const projExpenses = expenses.filter(
            (e: any) => e.projectName === pn && e.rowType === 'item',
          );
          ...
          for (const inf of projInflows) {
            ...
            if (!matchWeek && d >= weekDates[weekDates.length - 1])
              matchWeek = weekDates[weekDates.length - 1];
            if (matchWeek) {
          ...
          for (const exp of projExpenses) {
            const d =
              exp.adminDateOverride ||
              exp.expensePaymentDate ||
              exp.computedForecastPaymentDate ||
              exp.forecastPaymentDate ||
...
router.get(
  '/api/cashflow/planning-overrides',
  requireAuth,
  requirePermission('cashflow', 'view'),
  async (req, res) => {
    try {
...
router.post(
  ...
  requireAuth,
  requireAdmin,
  validateBody(planningOverridesSchema),
  async (req, res) => {
    try {
      ...
      if (!Array.isArray(overrides)) {
        return res
          .status(400)
          .json({ error: 'Overrides must be an array', message: 'Overrides must be an array' });
      }
      ...
      if (!overrideCategory || !OVERRIDE_CATEGORIES.includes(overrideCategory)) {
        return res
          ...
          .json({
            error:
              'Override category is required. Must be one of: ' + OVERRIDE_CATEGORIES.join(', '),
          });
      }
      if (
        !overrideComment ||
        typeof overrideComment !== 'string' ||
        overrideComment.trim().length < 3
      ) {
      ...
      const overridesWithUser = overrides.map((o: any) => {
        const numValue =
          typeof o.overrideValue === 'string' ? parseFloat(o.overrideValue) : o.overrideValue;
        if (isNaN(numValue)) {
...
router.delete(
  '/api/cashflow/planning-overrides/:projectName',
  requireAuth,
  requireAdmin,
  async (req, res) => {
    try {
...
router.post(
  ...
  validateBody(revenueTrackingOverridesSchema),
  async (req, res) => {
    try {
      const { overrides, overrideCategory, overrideComment } = req.body;
      if (!Array.isArray(overrides)) {
      ...
      const projectNames = [
        ...new Set(overrides.map((o: any) => o.projectName).filter(Boolean)),
      ] as string[];
      ...
      const baselineInflows =
        await financeInflowsRepository.listProgramInflowsByProjectNames(projectNames);
      ...
      try {
        ...
        for (const projectName of projectNames) {
          ...
          for (const r of appliedRows) {
            ...
            const hasInvoice = !!(
              r.milestoneInvoiceNumber && String(r.milestoneInvoiceNumber).trim()
            );
            const hasPaymentReceived = !!(
              r.paymentReceivedDate &&
              String(r.paymentReceivedDate).trim() &&
              r.paymentReceivedDate !== '-'
            );
            const confirmedByColor =
              typeof (r as any).paymentReceivedDateFontColor === 'string'
            ...
            const paidDate = isInBank
              ? r.paymentReceivedDate || r.plannedPaymentDate || null
              : null;
            ...
            await financeInflowsRepository.updatePaidDateFontColorById(
              canonicalRevId,
              paidDateFontColor,
            );
          }
        ...
        console.warn(
          '[sync] Revenue inBank sync to normalized_revenue_lines failed:',
          syncErr.message,
        );
      }
      ...
      res.json({
        message: 'Revenue tracking overrides saved',
        count: saved.length,
        overrides: saved,
      });
...
router.delete(
  '/api/revenue-tracking/overrides/:projectName',
  requireAuth,
  requireAdmin,
  async (req, res) => {
    try {
...
router.get('/api/revenue-tab/:projectName', requireAuth, async (req, res) => {
  try {
    ...
    try {
      useCanonical = await isWorkItemsEnabled();
    } catch (_e) {
    ...
    const [
      ...
      useCanonical
        ? getWorkItemsAsOperationalTasks(projectName).catch(() => [])
        : Promise.resolve([]),
    ...
    const operationalTasks =
      useCanonical && canonicalTasks.length > 0 ? canonicalTasks : legacyOperationalTasks;
    ...
    const overrideByFieldKey = new Map(
      overrides.map((row: any) => [`${row.rowNumber}|${row.fieldName}`, row]),
    );
    ...
    try {
      ...
      console.error(
        'Revenue tab: governance context failed, continuing with empty governance:',
        govError,
      );
    }
    ...
    const milestones = inflows.filter(isRealMilestone).map((r: any) => {
      ...
      const hasPaymentReceived = !!(
        r.paymentReceivedDate &&
        r.paymentReceivedDate.trim() &&
        r.paymentReceivedDate !== '-'
      );
      const confirmedByColor =
        typeof r.paymentReceivedDateFontColor === 'string'
      ...
      if (link) {
        if (link.taskId > 0) {
          linkedTask =
            operationalTasks.find((t: any) => t.id === link.taskId) ||
            legacyOperationalTasks.find((t: any) => t.id === link.taskId);
        } else {
          const planTask = planTasks.find((pt: any) => pt.id === Math.abs(link.taskId));
          if (planTask) {
            const pctComplete =
              (planTask as any).actualPctComplete != null
                ? Math.round((planTask as any).actualPctComplete * 100)
                : 0;
            ...
            linkedTask = {
              id: link.taskId,
              title:
                (planTask as any).highLevelProgramme ||
                `Task ${(planTask as any).taskNo || (planTask as any).rowNumber}`,
              status: taskStatus,
      ...
      const taskLinkChange = governance.latestChangeByEntity.get(
        `${projectName}|milestone${r.rowNumber}|task-link`,
      );
      const dateOverrideChange = governance.latestChangeByEntity.get(
        `${projectName}|milestone${r.rowNumber}|date-override`,
      );
      const fieldAudits = {
      ...
      const editedFields = Object.values(fieldAudits)
        .filter(
          (audit) =>
            auditValuesDiffer(audit.sourceValue, audit.managedValue) ||
            audit.overrideValue !== null ||
            audit.overrideComment,
        )
      ...
      const latestFieldAudit = Object.values(fieldAudits).reduce((latest: any, audit: any) => {
        ...
        return new Date(audit.changedAt).getTime() > new Date(latest.changedAt).getTime()
          ? audit
          : latest;
      }, null);
      const taskLinkChangedByName = formatChangeUserName(
        taskLinkChange?.actorUserId,
        taskLinkChange?.actorRole,
        governance.userNameById,
      );
      const dateOverrideChangedByName = formatChangeUserName(
        dateOverrideChange?.actorUserId,
        dateOverrideChange?.actorRole,
        governance.userNameById,
      );
      const lastChangedAt =
        [latestFieldAudit?.changedAt, taskLinkChange?.createdAt, dateOverrideChange?.createdAt]
          .filter(Boolean)
          .sort(
            (left, right) => new Date(String(right)).getTime() - new Date(String(left)).getTime(),
          )[0] || null;
      const lastChangedByName =
        latestFieldAudit?.changedByName ||
        dateOverrideChangedByName ||
        taskLinkChangedByName ||
        null;
    ...
    const totalContract = milestones.reduce(
      (s: number, m: any) => s + (parseFloat(m.milestoneAmount) || 0),
      0,
    );
    const invoiced = milestones
      .filter(
        (m: any) => m.status === 'invoiced' || m.status === 'inBank' || m.status === 'received',
      )
      .reduce((s: number, m: any) => s + (parseFloat(m.milestoneAmount) || 0), 0);
    const inBankTotal = milestones
      .filter((m: any) => m.status === 'inBank')
      .reduce((s: number, m: any) => s + (parseFloat(m.milestoneAmount) || 0), 0);
    const pending = milestones
      .filter((m: any) => m.status === 'planned' || m.status === 'overdue')
      .reduce((s: number, m: any) => s + (parseFloat(m.milestoneAmount) || 0), 0);
    ...
    try {
      ...
      for (const row of expenseRows) {
        if ((row as any).rowType === 'item') {
          ...
          const lineAmt =
            parseFloat(String((row as any).quotedTotal || (row as any).expenseActualTotal || 0)) ||
            0;
    ...
    const plannedRevenue = savedSummary?.plannedRevenue
      ? parseFloat(String(savedSummary.plannedRevenue))
      : contractValue || totalContract;
    const plannedExpenditureVal = savedSummary?.plannedExpenditure
      ? parseFloat(String(savedSummary.plannedExpenditure))
      : costedExpenditure;
    ...
    const costedChangedByName = formatChangeUserName(
      costedChange?.actorUserId,
      costedChange?.actorRole,
      governance.userNameById,
    );
    const overriddenMilestoneCount = new Set(overrides.map((row: any) => row.rowNumber)).size;
    const overriddenFieldCount = new Set(
      overrides.map((row: any) => `${row.rowNumber}:${row.fieldName}`),
    ).size;
    ...
    const riskSignals = [
      ...
      unlinkedMilestones.length > 0
        ? {
            ...
            amount: unlinkedMilestones.reduce(
              (sum: number, milestone: any) => sum + safeNum(milestone.milestoneAmount),
              0,
            ),
            count: unlinkedMilestones.length,
            detail:
              'Milestones should stay linked to project execution items for commercial traceability.',
          }
        : null,
    ...
    res.json({
      ...
      summary: {
        ...
        issueCount: milestones.filter(
          (m: any) => m.status === 'overdue' || m.status === 'invoiced' || !m.dependentTask,
        ).length,
      },
      highlevel: {
        costed: {
      ...
      reconciliation: {
        ...
        managed: {
          ...
          manualCostedOverride:
            !!savedSummary?.plannedRevenue || !!savedSummary?.plannedExpenditure,
...
router.post(
  '/api/revenue-tab/:projectName/costed',
  requireAuth,
  requireAdminOrFinancialEditor,
  validateBody(revenueTabCostedSchema),
  async (req, res) => {
    try {
      ...
      const auditCategory =
        typeof changeCategory === 'string' &&
        (OVERRIDE_CATEGORIES as readonly string[]).includes(changeCategory)
      ...
      const saved = await storage.upsertProjectRevenueSummary({
        ...
        plannedProfit:
          revenue && expenditure
            ? (parseFloat(revenue) - parseFloat(expenditure)).toString()
            : null,
        plannedMargin:
...
router.get(
  '/api/revenue-tab/:projectName/task-alerts',
  requireAuth,
  requireAdmin,
  async (req, res) => {
    try {
      ...
      const tasks =
        useCanonicalAlerts && canonicalAlertTasks.length > 0 ? canonicalAlertTasks : legacyTasks;
      ...
      for (const milestone of inflows) {
        ...
        const linkedTask = link
          ? tasks.find((t: any) => t.id === link.taskId) ||
            legacyTasks.find((t: any) => t.id === link.taskId)
          : null;

        if (
          linkedTask &&
          ((linkedTask as any).status === 'complete' ||
            (linkedTask as any).status === 'Complete') &&
          !milestone.milestoneInvoiceNumber
        ) {
          alerts.push({
...
router.post(
  '/api/revenue-tab/:projectName/link-task',
  requireAuth,
  requireAdminOrFinancialEditor,
  validateBody(revenueTabLinkTaskSchema),
  async (req, res) => {
    try {
      const projectName = paramStr(req.params.projectName);
      ...
      const previousLink = existingLinks.find(
        (link: any) => link.milestoneRowNumber === milestoneRowNumber,
      );
...
router.post(
  '/api/revenue-tab/:projectName/date-override',
  requireAuth,
  requireAdminOrFinancialEditor,
  validateBody(revenueTabDateOverrideSchema),
  async (req, res) => {
    try {
      const projectName = paramStr(req.params.projectName);
      ...
      if (link) {
        const updated = await storage.upsertMilestoneTaskLink(
          projectName,
          milestoneRowNumber,
          link.taskId,
        );
        await storage.updateMilestoneDateOverride(
          projectName,
          milestoneRowNumber,
          dateOverride,
          reason || null,
...
router.post(
  ...
  requireAdminOrFinancialEditor,
  requirePermission('financials', 'edit'),
  validateBody(expenditureOverridesSchema),
  async (req, res) => {
    try {
      const { overrides, overrideCategory, overrideComment } = req.body;
      if (!Array.isArray(overrides)) {
      ...
      if (isPmOnlyRole(userRole)) {
        const hasHighExpense = overrides.some(
          (o: any) => o.fieldName === 'expenseActualTotal' && Number(o.overrideValue) > 50000,
        );
...
router.delete(
  '/api/expenditure/overrides/:projectName',
  requireAuth,
  requireAdmin,
  async (req, res) => {
    try {
...
router.post(
  '/api/expense-task-links/:projectName',
  requireAuth,
  requireAdminOrFinancialEditor,
  validateBody(expenseTaskLinkSchema),
  async (req, res) => {
    try {
      ...
      const link = await storage.upsertExpenseTaskLink(
        paramStr(req.params.projectName),
        expenseId,
        taskId,
        (req.user as any)?.id,
      );
...
router.delete(
  '/api/expense-task-links/:projectName/:expenseId',
  requireAuth,
  requireAdminOrFinancialEditor,
  async (req, res) => {
    try {
...
router.post(
  '/api/expense-task-links/:projectName/:expenseId/date-override',
  requireAuth,
  requireAdminOrFinancialEditor,
  validateBody(expenseTaskLinkDateOverrideSchema),
  async (req, res) => {
    try {
      const { dateOverride, reason } = req.body;
      const expProjectName = paramStr(req.params.projectName);
      await storage.updateExpenseTaskLinkDateOverride(
        expProjectName,
        parseIntParam(req.params.expenseId),
        dateOverride,
        reason,
      );
...
router.post(
  '/api/expenses/add-line',
  requireAuth,
  requireAdmin,
  validateBody(addExpenseLineSchema),
  async (req, res) => {
    try {
      const {
        projectName,
...
router.post(
  '/api/expenses/add-category',
  requireAuth,
  requireAdmin,
  validateBody(addExpenseCategorySchema),
  async (req, res) => {
    try {
...
router.post(
  '/api/expenses/insert-task-as-line',
  requireAuth,
  requireAdmin,
  validateBody(insertTaskAsLineSchema),
  async (req, res) => {
    try {
      ...
      if (!projectName || !taskId || !expenseCategory) {
        return res
          .status(400)
          .json({ error: 'projectName, taskId, and expenseCategory are required' });
      }
      ...
      if (taskId > 0) {
        ...
        if (opTask) {
          taskTitle = opTask.title || '';
          taskEndDate = opTask.dueDate || null;
        }
      } else {
        const planTask = planTasks.find((t: any) => t.id === Math.abs(taskId));
        if (planTask) {
          taskTitle =
            (planTask as any).highLevelProgramme || `Task ${(planTask as any).taskNo || ''}`;
          taskEndDate = (planTask as any).actualEnd || null;
        }
      }
      const { rows: maxRow } = await getCanonicalProjectCostLinesByName(projectName);
      const maxRowNum = maxRow.reduce((max: number, r: any) => Math.max(max, r.rowNumber || 0), 0);
      ...
      await storage.upsertExpenseTaskLink(
        projectName,
        newExpense.id,
        taskId,
        (req.user as any)?.id,
...
router.get('/api/expenditure-breakdown/:projectName', requireAuth, async (req, res) => {
  try {
    const projectName = paramStr(req.params.projectName);
    const projectIdParam = req.query.projectId ? parseInt(String(req.query.projectId), 10) : null;
    const [projectLookup, taskLinks, opTasks, planTasks, projectRows, revSummary] =
      await Promise.all([
    ...
    const enriched = expenses
      ...
      .map((exp: any) => {
        ...
        if (link) {
          if (link.taskId > 0) {
            ...
            if (ot) {
              ...
              taskCompleted =
                ot.status === 'Complete' || ot.status === 'complete' || ot.status === 'Done';
            }
          } else {
            const pt = planTasks.find((t: any) => t.id === Math.abs(link.taskId));
            if (pt) {
              const pctComplete =
                (pt as any).actualPctComplete != null
                  ? Math.round((pt as any).actualPctComplete * 100)
                  : 0;
              let taskStatus = 'Not Started';
              if (pctComplete >= 100) {
                taskStatus = 'Done';
              ...
              linkedTask = {
                id: link.taskId,
                title:
                  (pt as any).highLevelProgramme ||
                  `Task ${(pt as any).taskNo || (pt as any).rowNumber}`,
                status: taskStatus,
        ...
        const invDateConfirmed =
          hasInvDate && isDateConfirmed(exp.invoiceDateConfirmed, exp.invoiceDateFontColor);
        ...
        const paymentDateBlack =
          hasPayDate &&
          !isFutureDate &&
          isDateConfirmed(exp.paymentDateConfirmed, exp.paymentDateFontColor);
        ...
        return {
          ...
          dateOverrideReason: link?.dateOverrideReason || null,
          cosOverride: cosOverride
            ? {
                reason: cosOverride.reason,
                overriddenBy: cosOverride.overriddenBy,
                originalStatus: cosOverride.originalStatus,
                overrideStatus: cosOverride.overrideStatus,
              }
            : null,
    ...
    const categories = [
      ...new Set(
        expenses
          .filter((e: any) => e.rowType === 'category')
          .map((e: any) => e.expenseCategory)
          .filter(Boolean),
      ),
    ];

    const importedBudgetRaw = enriched.reduce(
      (sum: number, item: any) => sum + safeNum(item.budgetTotal),
      0,
    );
    ...
    const totalActual = enriched.reduce(
      (sum: number, item: any) => sum + safeNum(item.expenseActualTotal),
      0,
    );
    ...
    const overBudgetItems = enriched.filter(
      (item: any) => safeNum(item.expenseActualTotal) > safeNum(item.budgetTotal),
    );
    const riskSignals = [
      ...
      committedUnpaidTotal > 0
        ? {
            ...
            count: enriched.filter(
              (item: any) => item.cosStatus === 'Committed' && item.paymentStatus !== 'Out of Bank',
            ).length,
...
router.post(
  ...
  requireAdmin,
  requirePermission('financials', 'edit'),
  validateBody(financeRevenueOverridesSchema),
  async (req, res) => {
    try {
      const { overrides, overrideCategory, overrideComment } = req.body;
      if (!Array.isArray(overrides)) {
      ...
      try {
        for (const o of overrides) {
          await recordOverride({
            actorUserId: userId,
            actorRole: (req as any).user?.role,
            entityType: 'finance_revenue_override',
            entityId: `${o.projectName}|row${o.rowNumber}|${o.fieldName}`,
            projectName: o.projectName,
            action: 'FINANCE_REVENUE_OVERRIDE',
            overrideCategory,
            overrideComment: overrideComment.trim(),
            oldRecord: {},
        ...
        console.warn('[audit] Finance revenue override audit failed:', auditErr.message);
      }

      res.json({
        message: 'Finance revenue overrides saved',
        count: saved.length,
        overrides: saved,
      });
      ...
      try {
        ...
        console.warn(
          '[finance] Finance revenue override metrics refresh failed:',
          metricsErr.message,
        );
      }
    } catch (error) {
...
router.delete(
  '/api/finance/revenue/overrides/:projectName',
  requireAuth,
  requireAdmin,
  async (req, res) => {
    try {
...
router.post(
  ...
  requireAdmin,
  requirePermission('financials', 'edit'),
  validateBody(financeCosOverridesSchema),
  async (req, res) => {
    try {
      const { overrides, overrideCategory, overrideComment } = req.body;
      if (!Array.isArray(overrides)) {
      ...
      try {
        for (const o of overrides) {
          await recordOverride({
            actorUserId: userId,
            actorRole: (req as any).user?.role,
            entityType: 'finance_cos_override',
            entityId: `${o.projectName}|row${o.rowNumber}|${o.fieldName}`,
            projectName: o.projectName,
            action: 'FINANCE_COS_OVERRIDE',
            overrideCategory,
            overrideComment: overrideComment.trim(),
            oldRecord: {},
            newRecord: { [o.fieldName]: o.overrideValue },
        ...
        console.warn('[audit] Finance COS override audit failed:', auditErr.message);
      }
...
router.delete(
  '/api/finance/cos/overrides/:projectName',
  requireAuth,
  requireAdmin,
  async (req, res) => {
    try {
