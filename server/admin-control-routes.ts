...
router.get("/api/admin/control-center/permission-enforcement", requireAuth, requireAdmin, async (req: Request, res: Response) => {
  try {
    const backendEnforced = [
      ...
      { route: "POST /api/pd/tickets", entity: "pd_tickets", action: "create", level: "requirePermission" },
      { route: "PATCH /api/pd/tickets/:id", entity: "pd_tickets", action: "edit", level: "requirePermission" },
      { route: "POST /api/pd/tickets/:id/spawn-tasks", entity: "pd_tickets", action: "edit", level: "requirePermission" },
