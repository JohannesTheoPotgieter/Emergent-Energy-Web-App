...
-- PROD PATCH: parent_work_item_id→parent_id, due_date→end_date, domain→source,
--             requester_user_id/approver_user_id/external_source/external_task_id missing → NULL
...
SELECT wi.id, wi.id, wi.project_id, wi.parent_id, wi.title, wi.description, wi.status, wi.priority, wi.phase,
       wi.owner_user_id, NULL, NULL, wi.end_date, wi.start_date, wi.completed_at,
       wi.sort_order, wi.source, NULL, NULL, wi.created_by, wi.created_at, wi.updated_at,
       'public.work_items'
...
-- PROD PATCH: Filter child rows to only include comments/attachments/activity
-- for operational_tasks that were actually inserted (i.e., not soft-deleted).
...
JOIN public.operational_tasks ot ON ot.id = tc.task_id AND ot.deleted_at IS NULL
...
JOIN public.operational_tasks ot ON ot.id = ta.task_id AND ot.deleted_at IS NULL
...
JOIN public.operational_tasks ot ON ot.id = tal.task_id AND ot.deleted_at IS NULL
...
-- PROD PATCH: deliverables.task_id and deliverables.created_by do not exist → use NULL
...
SELECT pi.id,
       ...
       pi.project_name,
       pi.milestone_name,
       pi.milestone_amount::NUMERIC(15,2),  -- column is already NUMERIC in prod (was TEXT when migration was written)
...
SELECT pe.id,
       ...
       pe.expense_actual_total::NUMERIC(15,2),  -- column is already NUMERIC in prod (was TEXT when migration was written)
